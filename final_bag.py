...
def main():
    ...
    for file in files:
        i=i+1
        outfile = "original"+str(i)+".bag"
        outbag = rosbag.Bag(outfile, 'w')
        data_accer=[]
        data_accer_t=[]
        data_gyr=[]
        data_gyr_t=[]
        with rosbag.Bag('bag_new.bag') as inbag:
                print ("-------------------- Input: %s ------------------------------" % file)
                print (inbag)
                for topic,msg,t in inbag.read_messages():
                    if topic=="/camera/accel/sample":
                        data_accer.append(msg)
                        data_accer_t.append(t)
        ...
        imu0=[]
        imu0_t=[]
        for acc, t in zip(data_accer, data_accer_t):
            imu=acc
            imu.angular_velocity=findAngular(data_gyr, data_gyr_t, t)
            outbag.write(imu0_topic, imu, t)
        for gyr, t in zip(data_gyr, data_gyr_t):
            imu=gyr
            imu.linear_acceleration=findGAccer(data_accer, data_accer_t, t)
            outbag.write(imu0_topic, imu, t)
...
def findAngular(data_gyr, data_gyr_t, target_t):
    imu=Imu()
    pre_t=data_gyr_t[0]
    last_t=data_gyr_t[0]
    pre_gyr=data_gyr[0]
...
def findGAccer(data_accer, data_accer_t, target_t):
    imu=Imu()
    pre_t=data_accer_t[0]
    last_t=data_accer_t[0]
    pre_accer=data_accer[0]
    last_accer=data_accer[0]
    for accer, t in zip(data_accer, data_accer_t):
        last_t=t
        last_accer=accer
        if target_t >=pre_t and target_t<=last_t:
            ratio=(last_t-target_t)/(last_t-pre_t)
